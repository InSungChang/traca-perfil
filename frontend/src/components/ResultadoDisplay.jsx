import {
  Star, AlertTriangle, BarChart3, Target,
  ChevronRight, Building2, Users, Sparkles
} from 'lucide-react'

function PlanCard({ period, actions }) {
  const styles = {
...
export default function ResultadoDisplay({ resultado, nome }) {
  const {
    perfil_geral,
    estilo_dominante,
    pontos_fortes = [],
  ...
  const nomeExibido = nome || 'do Mentorado'
      ...
      {cruzamento_perfil_empresa && (
        <div className="glass rounded-3xl p-7">
          ...
          {cruzamento_perfil_empresa.split('\n').map((p, i) => (
            <p key={i} className="text-sm text-ink-300 leading-relaxed mb-2 last:mb-0">{p}</p>
          ))}
        </div>
      )}
      ...
      {analise_talentos && (
        <div className="glass rounded-3xl p-7">
          ...
          {analise_talentos.split('\n').map((p, i) => (
            <p key={i} className="text-sm text-ink-300 leading-relaxed mb-3 last:mb-0">{p}</p>
          ))}
        </div>
      )}
